...
def main(unused_argv=None):
  ...
  with tf.Graph().as_default(), tf.Session() as sess:
    ...
    for content_i, content_img_path in enumerate(content_img_list):
      ...
      #saving content images
      image_utils.save_np_image(inp_img_croped_resized_np,
                                os.path.join(FLAGS.output_dir+ "content/",
                                             'content_%s.jpg' % (content_i)))
      ...
      #changed to pairwise transfer  
      #forwardonly# 
      style_img_path = style_img_list[style_i]
      ...
      image_utils.save_np_image(style_img_croped_resized_np,
                                os.path.join(FLAGS.output_dir+"style/",
                                             'style_%s.jpg' % (style_i)))
      ...
      for interp_i, wi in enumerate(interpolation_weights):
        ...
        image_utils.save_np_image(
            stylized_image_res,
            os.path.join(FLAGS.output_dir+ "stylized/", 'stylized_%s.jpg' %
                         (content_i)))
    ...
    #     # for backward
    # for content_i in range(30):
    #   style_i = content_i

    #   content_img_list = tf.gfile.Glob(FLAGS.content_images_paths + "stylized_" + str(content_i) + ".jpg")
    #   content_img_path = content_img_list[0]

    #   style_img_list = tf.gfile.Glob(FLAGS.style_images_paths + "content_" + str(style_i) + ".jpg")
    #   style_img_path = style_img_list[0]
      

    #   #forward
    #   # content_img_path = "traindata4/train0/stylized/stylized_" + str(content_i) + ".jpg"
    #   # if content_i % 999 == 0:
    #   #   j = content_i // 999
    #   #   # Gets the list of the input style images.
    #   #   style_img_list = tf.gfile.Glob(FLAGS.style_images_paths + "painter" + str(j))
    #     # if len(style_img_list) > FLAGS.maximum_styles_to_evaluate:
    #     #   np.random.seed(1234)
    #     #   style_img_list = np.random.permutation(style_img_list)
    #     #   style_img_list = style_img_list[:FLAGS.maximum_styles_to_evaluate]  
    ...
    #     image_utils.save_np_image(
    #         stylized_image_res,
    #         os.path.join(FLAGS.output_dir+ "identity/", 'identity_%s.jpg' %
    #                      (content_i)))
